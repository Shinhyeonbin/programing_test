...
class Sort{
	...
	static class Merge_sort{
		...
		private void merge_sort(int[] a, int left, int right) {
			...
			merge_sort(a, left, middle);	//divide left
			merge_sort(a, middle + 1, right);	//divide right
			
			merge(a, left, middle, right);	//merge_processing
		...
		private void merge(int[] a, int left, int middle, int right) {
			...
			while(l_idx <= middle && r_idx <= right) {
				if(a[l_idx] <= a[r_idx]) {	//left index value is bigger than right
					sorted[index] = a[l_idx];	//store temp_arr
					l_idx++;
					index++;
				}
				else {
...
class Graph{
	...
	int size;
	boolean visit[];
	private int maps[][];	//To do shortest_path
...
//Longest_path code referenced this site https://www.geeksforgeeks.org/find-longest-path-directed-acyclic-graph/
...
public class hw1 {
	private static int[] random(int size) {	//To test Sort, Search
	int[] arr = new int[10];
